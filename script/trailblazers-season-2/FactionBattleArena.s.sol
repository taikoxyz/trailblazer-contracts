...
import
    "../../contracts/trailblazers-season-2/TrailblazersS1BadgesV8.sol";
...
contract FactionBattleArenaDeployScript is Script {
  ...
  s2Token = TrailblazersBadgesS2(0x52A7dBeC10B404548066F59DE89484e27b4181dA);
        ...
        FactionBattleArena.Config memory config = FactionBattleArena.Config(
           CONFIG_LEAGUE_DURATION,
            address(s1Token),
            address(s2Token)
        );
        ...
        impl = address(new FactionBattleArena());
            ...
            proxy = address(
                new ERC1967Proxy(
                    impl,
                    abi.encodeCall(
                        FactionBattleArena.initialize, (config)
                    )
                )
            );
        ...
        string memory finalJson = vm.serializeAddress(jsonRoot, "FactionBattleArena", address(arena));
